package com.incretio.creditmanager.demo;
...
public class AnnuityCredit extends Credit {
    ...
    public static void main(String[] args) {
        Credit credit = new AnnuityCredit(2934000.0d, 800000.0d, new GregorianCalendar(2016, Calendar.OCTOBER, 18).getTime(), 180, 11.9d);
        ...
        for (CreditGraphicRecord creditGraphicRecord : credit.getCreditGraphic()) {
            System.out.println(String.format("%d\t%s\t%s\t%s\t%s\t%s\t%s", creditGraphicRecord.getMonthNumber(),
                                             DateUtils.formatDate(creditGraphicRecord.getPaymentDate()),
                                             Credit.formatDouble(creditGraphicRecord.getPrincipalPayment()),
                                             Credit.formatDouble(creditGraphicRecord.getInterestPayment()),
                                             Credit.formatDouble(creditGraphicRecord.getPartialEarlyPayment()),
                                             Credit.formatDouble(creditGraphicRecord.getMonthlyPayment()),
                                             Credit.formatDouble(creditGraphicRecord.getPrincipalBalance())));
        }
    }
}